            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-header-left">
                        <h5 class="text-uppercase title">List Transaksi Reward</h5>
                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        $("#formRejectSuccess").submit(function(e) {
            ...
            if (c) {
                ...
                if ($("#reqStatus").val() == "SUCCESS") {
                    dataToSend.append("proof_of_acception", document.getElementById("proofPayment").files[0]);
                }
        ...
        function sendChangeStatus(data, hideModal = false) {
            $.ajax({
                url: "{{ route('trx-reward.change-status') }}",
                contentType: false,
                processData: false,
                method: "POST",
                data: data,
        ...
        function exportData() {
            ...
            window.location.href = `{{ route('export.trx-reward') }}?${$.param(dataFilter)}`
        }
    </script>
@endpush
